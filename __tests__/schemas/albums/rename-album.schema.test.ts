...
describe("RenameAlbumSchema", () => {
  it("parses a valid payload", () => {
    expect(
      RenameAlbumSchema.safeParse({ albumId: UUID, newTitle: "New Name" }).success
    ).toBe(true);
  });
  ...
  it("rejects an empty title", () => {
    ...
    if (!result.success)
      expect(result.error.issues[0].message).toBe("Album title is required");
  });
  ...
  it("rejects a title over 200 characters", () => {
    ...
    if (!result.success)
      expect(result.error.issues[0].message).toBe(
        "Album title must be 200 characters or fewer"
      );
  });

  it("accepts a title of exactly 200 characters", () => {
    expect(
      RenameAlbumSchema.safeParse({ albumId: UUID, newTitle: "a".repeat(200) }).success
    ).toBe(true);
  });
