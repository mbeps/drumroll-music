...
describe("RenamePlaylistSchema", () => {
  it("parses a valid payload", () => {
    expect(
      RenamePlaylistSchema.safeParse({ playlistId: UUID, newTitle: "Road Trip" })
        .success
    ).toBe(true);
  });
  ...
  it("rejects a title over 100 characters", () => {
    expect(
      RenamePlaylistSchema.safeParse({ playlistId: UUID, newTitle: "x".repeat(101) })
        .success
    ).toBe(false);
  });

  it("rejects an invalid UUID", () => {
    expect(RenamePlaylistSchema.safeParse({ playlistId: "x", newTitle: "A" }).success).toBe(
      false
    );
  });
});
