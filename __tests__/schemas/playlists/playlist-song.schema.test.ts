...
describe("PlaylistSongSchema", () => {
  it("parses a valid payload", () => {
    expect(PlaylistSongSchema.safeParse({ playlistId: UUID, songId: 1 }).success).toBe(
      true
    );
  });
  ...
  it("rejects a negative song ID", () => {
    expect(PlaylistSongSchema.safeParse({ playlistId: UUID, songId: -3 }).success).toBe(
      false
    );
  });

  it("rejects a non-integer song ID", () => {
    expect(PlaylistSongSchema.safeParse({ playlistId: UUID, songId: 1.5 }).success).toBe(
      false
    );
  });
});
