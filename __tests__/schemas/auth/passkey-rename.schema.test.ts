...
describe("PasskeyRenameSchema", () => {
  it("parses a valid payload", () => {
    expect(PasskeyRenameSchema.safeParse({ passkeyId: UUID, newName: "MacBook" }).success).toBe(
      true,
    );
  });
  ...
  it("rejects an empty name", () => {
    ...
    if (!result.success) expect(result.error.issues[0].message).toBe("Passkey name is required");
  });
  ...
  it("rejects a name over 50 characters", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success)
      expect(result.error.issues[0].message).toBe("Passkey name must be 50 characters or fewer");
  });
